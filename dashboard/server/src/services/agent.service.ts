...
export function syncAgentsToFileSystem() {
    ...
    try {
      ...
      for (const row of rows) {
            ...
            prevAgents.forEach((id) => {
              ...
              if (outputs && outputs.length > 0) {
                outputs.forEach((out: string) => {
                  prevOutputs.push(
                    `- \`${out.replace("output/", "")}\` from ${teamConfig.members[id].role}`,
                  );
                });
              }
            });
        ...
        if (!finalInstructions.includes("## Output Format")) {
          ...
          const outputFiles =
            outputs && outputs.length > 0 ? outputs.join(", ") : "output.md";
...
export function removeAgentPrompt(id: string) {
  try {
