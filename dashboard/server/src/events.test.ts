...
describe("setupSocketEvents", () => {
  ...
  describe("state:resync", () => {
    it("should send full state again on state:resync event", () => {
      ...
      fs.writeFileSync(
        path.join(flowDir, "workflow.json"),
        JSON.stringify(
          makeWorkflow({ flowId: "flow_002", jiraKey: "RESYNC-1" }),
        ),
      );
      ...
      expect(socket.emit).toHaveBeenCalledWith("state:init", {
        flows: {
          flow_002: expect.objectContaining({
            flowId: "flow_002",
            jiraKey: "RESYNC-1",
          }),
        },
  ...
  describe("workspace:select", () => {
    it("should send state:init with flows from selected workspace", () => {
      ...
      fs.writeFileSync(
        path.join(flowDir, "workflow.json"),
        JSON.stringify(
          makeWorkflow({ flowId: "flow_workspace_001", jiraKey: "WS-1" }),
        ),
      );
      ...
      socket.triggerEvent("workspace:select", {
  ...
  describe("log:subscribe / log:unsubscribe", () => {
    it("should join the log room on log:subscribe", () => {
      ...
      const socket = simulateConnect();
      socket.triggerEvent("log:subscribe", {
        flowId: "flow_001",
        step: "clarifier",
      });
    ...
    it("should leave the log room on log:unsubscribe", () => {
      ...
      const socket = simulateConnect();
      socket.triggerEvent("log:unsubscribe", {
        flowId: "flow_001",
        step: "planner",
      });
  ...
  describe("watcher event wiring", () => {
    it("should broadcast flow:updated when watcher emits workflow-changed", () => {
      ...
      expect(io.emit).toHaveBeenCalledWith("flow:updated", {
        flowId: "flow_x",
        workflow,
      });
    });
