...
import { readWorkflowJson, readOutputContent, listAllFlows } from './flow-reader.js';
...
describe('readWorkflowJson', () => {
  ...
  it('should parse a valid workflow.json', () => {
    ...
    fs.writeFileSync(path.join(tmpDir, 'workflow.json'), JSON.stringify(workflow));
...
describe('listAllFlows', () => {
  ...
  it('should scan all valid flow directories', () => {
    ...
    fs.writeFileSync(
      ...
      JSON.stringify(makeWorkflow({ flowId: 'flow_002', jiraKey: 'A-2', status: 'completed' }))
    );
