...
vi.mock("../db.js", () => ({
  db: { get: vi.fn((q, p, cb) => cb(null, null)) },
}));
...
function createMockDashboardRoot(): { rootDir: string; taskFlowsDir: string } {
  ...
  const taskFlowsDir = path.join(rootDir, "task-flows");
  const scriptsDir = path.join(rootDir, "scripts");
  const libDir = path.join(scriptsDir, "lib");
  fs.mkdirSync(taskFlowsDir, { recursive: true });
  fs.mkdirSync(scriptsDir, { recursive: true });
  fs.mkdirSync(libDir, { recursive: true });
  fs.mkdirSync(path.join(scriptsDir, "orchestrator"), { recursive: true });
  fs.mkdirSync(path.join(scriptsDir, "api"), { recursive: true });
  fs.mkdirSync(path.join(scriptsDir, "watcher"), { recursive: true });
...
exports.prepareRetry = function prepareRetry(flowId, step) {
  ...
  return { workDir: path.join(rootDir, 'task-flows', flowId) };
};
`,
  );
...
describe("flowsRouter", () => {
  ...
  describe("GET /api/flows/:flowId/logs/:step", () => {
    it("returns empty lines when log does not exist", async () => {
      ...
      const { status, body } = await request(
        app,
        "GET",
        "/api/flows/flow_004/logs/clarifier",
      );
      expect(status).toBe(200);
    ...
    it("returns log lines", async () => {
      ...
      fs.writeFileSync(
        path.join(logsDir, "clarifier.log"),
        "line1\nline2\nline3",
      );
      ...
      const { status, body } = await request(
        app,
        "GET",
        "/api/flows/flow_005/logs/clarifier",
      );
      expect(status).toBe(200);
    ...
    it("returns only last 1000 lines for large logs", async () => {
      ...
      const { status, body } = await request(
        app,
        "GET",
        "/api/flows/flow_006/logs/architect",
      );
      expect(status).toBe(200);
  ...
  describe("GET /api/flows/:flowId/output/:step", () => {
    it("returns 400 for invalid step", async () => {
      ...
      const { status, body } = await request(
        app,
        "GET",
        "/api/flows/flow_007/output/invalid",
      );
    ...
    it("returns exists: false when output file does not exist", async () => {
      ...
      const { status, body } = await request(
        app,
        "GET",
        "/api/flows/flow_008/output/clarifier",
      );
      expect(status).toBe(200);
    ...
    it("returns markdown content and metadata", async () => {
      ...
      fs.writeFileSync(
        path.join(outputDir, "clarify.md"),
        "# Clarification\n\nSome content.",
      );
      ...
      const { status, body } = await request(
        app,
        "GET",
        "/api/flows/flow_009/output/clarifier",
      );
      expect(status).toBe(200);
  ...
  describe("GET /api/flows/:flowId/tokens", () => {
    ...
    it("resolves token data for flows inside a workspace directory", async () => {
      ...
      fs.writeFileSync(
        path.join(flowDir, "logs", "analyzer.log"),
        "tokens used\n12,345\n",
      );
    ...
    it("starts workflow without requiring node to be resolvable from PATH", async () => {
      ...
      try {
        ...
        const args = JSON.parse(
          fs.readFileSync(
            path.join(rootDir, "scripts", "orchestrator-args.json"),
            "utf8",
          ),
        );
  ...
  describe("POST /api/flows/:flowId/retry", () => {
    it.skip("restarts watcher after retrying a step", async () => {
      ...
      fs.mkdirSync(path.join(taskFlowsDir, flowId, "logs"), {
        recursive: true,
      });

      const app = makeApp(createMockConfig(taskFlowsDir));
      const watcherScript = path.join(rootDir, "scripts", "watcher/index.js");
      const existingWatcher = spawn(
        process.execPath,
        [watcherScript, flowId, "existing"],
        {
          stdio: "ignore",
        },
      );
      ...
      try {
        ...
        await waitForFileContent(watcherStartsPath, (content) =>
          content.includes(`existing:${flowId}:`),
        );

        const { status, body } = await request(
          app,
          "POST",
          `/api/flows/${flowId}/retry`,
          {
            step: "architect",
            clearOutput: true,
          },
        ...
        const watcherStarts = await waitForFileContent(
          ...
          (content) =>
            content.includes(`existing:${flowId}:`) &&
            content.includes(`new:${flowId}:`),
        );
        expect(watcherStarts).toContain(`new:${flowId}:`);

        const retryCalls = fs.readFileSync(
          path.join(rootDir, "retry-calls.log"),
          "utf8",
        );
