...
function buildStatePayload(
  ...
  const dir = workspaceName
    ? path.join(config.taskFlowsDir, workspaceName)
    : config.taskFlowsDir;
...
export function setupSocketEvents(
  ...
  io.on("connection", (socket: Socket<any, any>) => {
    ...
    socket.on(
      "workspace:select",
      ({ workspaceName }: { workspaceName: string | null }) => {
    ...
    socket.on(
      "log:subscribe",
      ({ flowId, step }: { flowId: string; step: string }) => {
  ...
  if (watcher) {
    ...
    watcher.on(
      "output-updated",
      (flowId: string, step, content: string, metadata) => {
