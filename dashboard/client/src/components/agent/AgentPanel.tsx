...
function useFlowStepData(flowId: string | null, workspaceName?: string) {
  ...
  useEffect(() => {
    ...
    fetch(
      `${API_BASE}/api/flows/${encodeURIComponent(flowId)}/tokens${query}`,
...
export function AgentPanel() {
  ...
  const { perStep, total, outputTimes } = useFlowStepData(
    selectedFlowId,
    workspaceName,
  );
      ...
      <div className="flex items-center justify-center py-12">
        <div className="text-center space-y-2">
          ...
          <p className="text-sm text-muted-foreground">
            Select a flow to view pipeline
          </p>
        </div>
      </div>
    );
            ...
            {(flow.stepOrder || AGENT_STEPS).map((step) => {
              ...
              return (
                ...
                >
                  ...
                  {/* Info */}
                  <td
                    className="py-2.5 text-muted-foreground/70 truncate max-w-[180px]"
                    title={info}
                  >
                    {info}
                  </td>
      ...
      {flow.status === "blocked" && flow.blockedReason && (
        <div className="flex items-center gap-2 rounded-lg bg-purple-500/10 border border-purple-500/20 px-3 py-2">
          ...
          <span className="text-xs text-purple-300">
            <span className="font-medium">Blocked:</span> {flow.blockedStep} —{" "}
            {flow.blockedReason}
          </span>
        </div>
      )}
