...
export function FlowActions() {
  ...
  const handleImprovePrompt = async () => {
    if (!retryPromptText.trim()) return;
    setImproving(true);
    setImproveError(null);
    try {
      const res = await fetch(`${API_BASE}/api/improve-prompt`, {
        method: "POST",
        headers: { "Content-Type": "application/json" },
        body: JSON.stringify({ prompt: retryPromptText }),
      });
      const data = await res.json();
      if (res.ok && data.improved) {
        setRetryPromptText(data.improved);
      } else {
          ...
          <div className="bg-card border border-border rounded-xl shadow-2xl w-full max-w-lg p-5 space-y-4">
            ...
            <div className="flex items-center justify-between">
              <button
                ...
                disabled={improving || !retryPromptText.trim()}
                className={cn(
              ...
              {improveError && (
                <span className="text-xs text-red-400">{improveError}</span>
              )}
            </div>
