...
export function FlowActions() {
  ...
  const handleStop = async () => {
    if (!confirm(`Stop workflow ${flow.flowId}? This will kill all running agents.`)) return;

    setLoading('stop');
    try {
      const res = await fetch(`${API_BASE}/api/flows/${flow.flowId}/stop`, { method: 'POST' });
  ...
  const handleRetry = async (step: AgentStep, clearOutput: boolean) => {
    ...
    try {
      const res = await fetch(`${API_BASE}/api/flows/${flow.flowId}/retry`, {
        ...
        body: JSON.stringify({ step, clearOutput }),
      });
      const data = await res.json();
      if (res.ok) {
        showMessage('success', data.message);
              ...
              {(flow.stepOrder || AGENT_STEPS).map((step) => (
                ...
                >
                  ...
                  <span className={cn(
                    ...
                    flow.steps[step] === 'running' && 'bg-blue-500/10 text-blue-400',
                    flow.steps[step] === 'failed' && 'bg-red-500/10 text-red-400',
                    flow.steps[step] === 'blocked' && 'bg-purple-500/10 text-purple-400',
                    flow.steps[step] === 'waiting' && 'bg-gray-500/10 text-gray-400',
                  )}>
                    {flow.steps[step]}
                  </span>
                </button>
            ...
            <div className="absolute bottom-full left-0 mb-1 z-50 w-96 max-w-screen-lg rounded-xl border border-border bg-card shadow-xl p-3 space-y-3">
              ...
              <div className="flex items-center justify-between pb-2 border-b border-border/50">
                <span className="text-xs font-medium text-foreground">
                  Git Status - {gitStatus.repos.length} {gitStatus.repos.length === 1 ? 'repository' : 'repositories'}
                </span>
                <span className="text-[10px] text-muted-foreground">
                  {gitStatus.repos.reduce((sum, repo) => sum + repo.files.length, 0)} total changes
                </span>
              </div>
              ...
              <div className="space-y-2 max-h-96 overflow-y-auto">
                {gitStatus.repos.map((repo, repoIdx) => (
                  <div key={repoIdx} className="rounded-lg border border-border/30 bg-muted/20 p-2.5 space-y-2">
                    {/* Repo header */}
                    ...
                    {repo.branch && (
                      <div className="text-[11px] text-muted-foreground">
                        Branch: <span className="text-blue-400 font-mono">{repo.branch}</span>
                      </div>
                    )}
                    ...
                    {repo.files.length > 0 && (
                      <div className="max-h-32 overflow-y-auto rounded-lg bg-muted/50 p-2 space-y-0.5">
                        {repo.files.slice(0, 30).map((file, fileIdx) => (
                          <div key={fileIdx} className="text-[11px] font-mono text-muted-foreground truncate">
                            <span className={cn(
