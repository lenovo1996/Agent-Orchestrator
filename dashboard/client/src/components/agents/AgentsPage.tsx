...
export function AgentsPage() {
  ...
  return (
    <div className="p-6 mx-auto h-full overflow-y-auto">
      <div className="flex justify-between items-center mb-8">
        <div>
          ...
          <p className="text-sm text-muted-foreground mt-1">
            Configure and manage AI agents for your workflows.
          </p>
        </div>
        {!isEditing && (
          <button
      ...
      {error && (
        <div className="mb-4 p-3 bg-red-500/10 text-red-500 border border-red-500/20 rounded">
          {error}
        </div>
      )}
        ...
        >
          <div className="flex items-center gap-2 border-b border-border/50 pb-4">
            ...
            <h2 className="text-lg font-semibold">
              {isNew ? "Create New Agent" : "Edit Agent Configuration"}
            </h2>
          </div>
        ...
        <div className="grid grid-cols-1 lg:grid-cols-2 gap-4">
          {agents.map((agent) => (
            ...
            >
              ...
              <p
                className="text-sm text-muted-foreground mb-4 flex-1 line-clamp-2"
              ...
              <div className="flex flex-wrap gap-2 text-[11px] mt-auto pt-4 border-t border-border/40">
                ...
                {agent.tools.length > 0 && (
                  <span
                    className="flex items-center gap-1 text-muted-foreground bg-muted/50 px-2 py-1 rounded-md border border-border/50"
                ...
                {agent.outputs.length > 0 && (
                  <span
                    className="flex items-center gap-1 text-muted-foreground bg-muted/50 px-2 py-1 rounded-md border border-border/50"
                    title={agent.outputs.join(", ")}
                  >
                    <FileOutput className="w-3 h-3" />
                    {agent.outputs.length} output
                    {agent.outputs.length !== 1 ? "s" : ""}
                  </span>
                )}
              </div>
            </div>
