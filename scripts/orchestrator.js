...
const { prepareRetry } = require('./lib/retry-flow');
const { ParallelScheduler } = require('./lib/parallel-scheduler');
const { detectRepos } = require('./lib/repo-detector');
const { initTree } = require('./memory-tree');
...
function startWorkflow(jiraKey = '', customPrompt = '') {
  ...
  const spawnScript = path.join(__dirname, 'spawn-via-gateway.js');
