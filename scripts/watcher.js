...
const { updateTree } = require('./memory-tree');
const { parseStepTokens, formatTokens } = require('./lib/token-tracker');
...
function spawnStep(flowId, step, isRetry = false) {
  ...
  const spawnScript = path.join(SCRIPT_DIR, 'spawn-via-gateway.js');
      ...
      if (currentStatus && currentStatus !== lastStatus) {
        ...
        if (currentStatus === 'DONE') {
          ...
          if (nextStep && workflow.steps[nextStep] === 'waiting') {
            ...
            const { getFlowTokens: getFlowTokensFn } = require('./lib/token-tracker');
...
function emitSummary(flowResults, startTime) {
  ...
  Object.entries(flowResults).forEach(([flowId, result]) => {
    ...
    const { flowTotal } = require('./lib/token-tracker').getFlowTokens(flowId);
